...
// Se afiseaza toate inregistrarile continute in tabele ap_electrice si se afiseaza puterile nominale
// Listarea se poate face dupa mai multe criterii de ordonare
$page_title = 'Putere nominala aparate electrice';
include ('./includes/header.php');
// Includere fisier  meniu.php.
include ('./includes/meniu.php');
echo '<body bgcolor="8fbc8f">';
echo '<div id="Continut_p">';
echo '<b><font color="blue" size="2">Caracteristici aparate electrice</font></b><br>';
...
<tr bgcolor="Purple">
	...
	<td align="center"><font color="white" size="2">P aparenta (VA)</td>
	<td align="center"><font color="white" size="2">P activa (WATT)</td>
	<td align="center"><font color="white" size="2">P reactiva (VAR)</td>
</tr>
';
...
while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	...
	echo '<tr bgcolor="' . $bg . '">
		...
		<td align="center"><h5>' . number_format($row['tens_n']*$row['crnt_n'],2) . '</td>
		<td align="center"><h5>' . number_format($row['tens_n']*$row['crnt_n']*cos($row['def']),2) . '</td>
		<td align="center"><h5>' . number_format($row['tens_n']*$row['crnt_n']*sin($row['def']),2) . '</td>
	</tr>
	';
}
...
if ($num_pages > 1) {
	...
	if ($current_page != 1) {
		echo '<a href="p_electr.php?s=' . ($start - $display) . '&np=' . $num_pages . '&sort=' . $sort .'">Pagina precendenta</a> ';
	}
	
	// Se pun numerele de pagina.
	for ($i = 1; $i <= $num_pages; $i++) {
		if ($i != $current_page) {
			echo '<a href="p_electr.php?s=' . (($display * ($i - 1))) . '&np=' . $num_pages . '&sort=' . $sort .'">' . $i . '</a> ';
		} else {
			echo $i . ' ';
		}
	}
	...
	if ($current_page != $num_pages) {
		echo '<a href="p_electr.php?s=' . ($start + $display) . '&np=' . $num_pages . '&sort=' . $sort .'">Pagina urmatoare</a>';
	}
